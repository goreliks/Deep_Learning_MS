...
def l2_dist(x1: Tensor, x2: Tensor):
    ...
    x2_sq = torch.sum(x2 ** 2, dim=1)
    x1_x2 = torch.mm(x1, x2.t())
    squared_dists = x1_sq - 2 * x1_x2 + x2_sq

    # Take square root to obtain L2 distances
...
def find_best_k(ds_train: Dataset, k_choices, num_folds):
    ...
    for i, k in enumerate(k_choices):
        ...
        for fold in range(num_folds):
            dl_train, dl_valid = dataloaders.create_train_validation_loaders(ds_train,
                                                                             validation_ratio)
