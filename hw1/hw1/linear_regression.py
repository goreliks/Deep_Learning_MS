...
def top_correlated_features(df: DataFrame, target_feature, n=5):
    ...
    # ====== YOUR CODE: ======
    corr_matrix = df.corr()
    corrs = corr_matrix[target_feature].drop(target_feature)
    corrs = corrs.apply(abs)
    top_n = corrs.sort_values(ascending=False).head(n)
    top_n_features = top_n.index.tolist()
    top_n_corr = top_n.values.tolist()
    # ========================
