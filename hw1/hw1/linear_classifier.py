...
class LinearClassifier(object):
    ...
    def train(
        ...
        for epoch_idx in range(max_epochs):
            ...
            for batch in dl_train:
                ...
                loss = loss_fn(x_train, y_train, class_scores, y_pred)
                grad = loss_fn.grad()
                reg_term = weight_decay * self.weights
                self.weights -= learn_rate * (grad + reg_term)
                total_correct += self.evaluate_accuracy(y_train, y_pred).item()
            ...
            for batch in dl_valid:
                ...
                loss = loss_fn(x_valid, y_valid, class_scores, y_pred)
                total_correct += self.evaluate_accuracy(y_valid, y_pred).item()
                average_loss += loss.item()
    ...
    def weights_as_images(self, img_shape, has_bias=True):
        ...
        # ====== YOUR CODE: ======
        if has_bias:
            return self.weights[1:].view(self.n_classes, *img_shape)
        else:
            return self.weights.view(self.n_classes, *img_shape)
        # ========================
