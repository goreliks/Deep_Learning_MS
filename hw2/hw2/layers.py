...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        # ====== YOUR CODE: ======
        softmax = torch.exp(x) / torch.exp(x).sum(dim=1).view(-1, 1)
        loss = -torch.log(softmax[range(N), y])
    ...
    def backward(self, dout=1.0):
        ...
        # ====== YOUR CODE: ======
        dx = torch.exp(x) / torch.exp(x).sum(dim=1).view(-1, 1)
        dx[range(N), y] -= 1
        dx = dx / N
        # ========================
