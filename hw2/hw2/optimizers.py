...
class MomentumSGD(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, momentum=0.9):
        ...
        for _, dp in self.params:
            self.vel[id(dp)] = torch.zeros_like(dp)
        # ========================
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            idp = id(dp)
            self.vel[idp] = self.momentum * self.vel[idp] - self.learn_rate * (dp + self.reg * p)
            p += self.vel[idp]
            # ========================
...
class RMSProp(Optimizer):
    def __init__(self, params, learn_rate=1e-3, reg=0, decay=0.99, eps=1e-8):
        ...
        self.r = {}
        for _, dp in self.params:
            self.r[id(dp)] = torch.zeros_like(dp)
        # ========================
    ...
    def step(self):
        for p, dp in self.params:
            ...
            # ====== YOUR CODE: ======
            idp = id(dp)
            self.r[idp] = self.decay * self.r[idp] + ((1-self.decay) * (torch.pow(dp + self.reg * p, 2)))
            p -= (self.learn_rate * (dp + self.reg * p)) / torch.sqrt(self.r[idp] + self.eps)
            # ========================
